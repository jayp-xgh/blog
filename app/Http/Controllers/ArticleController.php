...
class ArticleController extends Controller
{
    ...
    {
        $dados = $articleUpdateRequest->all();

        if ($articleUpdateRequest->file('image')) {
            @unlink(public_path('img/articles/' . $article->image));
            $file_name = time() . '.' . $articleUpdateRequest->file('image')->getClientOriginalExtension();
            $articleUpdateRequest->file('image')->move(public_path('img/articles/'), $file_name);
            $dados['image'] = $file_name;
        }

        $article->update($dados);
